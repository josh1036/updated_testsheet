...
/* ── Shared sub-components (used in both on-screen view and PDF target) ── */
...
/**
 * RecordBody — the actual record content shared between the on-screen view and the PDF render target.
 * Keeping it in one place means the PDF is always pixel-identical to what the user sees on screen.
 */
function RecordBody({ record, mainsRows, subRows }) {
      ...
      {hasCompanyBranding && (
        <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-start gap-4">
          {record.companyLogoUrl && (
            <img src={record.companyLogoUrl} alt="Company logo" className="h-16 max-w-[160px] object-contain" />
          )}
          <div>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
        ...
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            {!shareMode && (
              <button
                ...
                className="flex items-center gap-1.5 text-slate-500 hover:text-[#0f2044] text-sm transition-colors"
              >
                <ArrowLeft className="w-4 h-4" /> Dashboard
              </button>
            )}
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-[#0f2044] flex items-center justify-center">
                <Zap className="w-3 h-3 text-[#F59E0B]" />
              </div>
              <span className="font-semibold text-slate-700 text-sm truncate">
                {record.addressLocation || 'Test Record'}
              </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 text-slate-700 text-sm hover:border-[#0f2044] transition-colors no-print"
            >
              <Printer className="w-4 h-4" /> Print
            </button>
            <button
              onClick={handleSavePdf}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0f2044] text-white text-sm hover:bg-[#162d4a] transition-colors no-print"
            >
              <Download className="w-4 h-4" /> Save PDF
            </button>
            {!shareMode && params.id && (
              <button
                onClick={() => navigate(`/records/${params.id}/edit`)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#0f2044] text-[#0f2044] text-sm hover:bg-blue-50 transition-colors no-print"
              >
      ...
      {/*
        ...
        Uses the same RecordBody component as the on-screen view — ONE SOURCE OF TRUTH.
        The PDF will look identical to what the user sees on screen.
      */}
      <div
        ref={reportRef}
        style={{
        ...
        aria-hidden="true"
      >
        {record && <RecordBody record={record} mainsRows={mainsRows} subRows={subRows} />}
      </div>

      {/* Visible on-screen record content */}
      <div className="max-w-5xl mx-auto px-4 py-6 print-page">
        <RecordBody record={record} mainsRows={mainsRows} subRows={subRows} />

        {/* Action buttons (not included in PDF) */}
        <div className="flex justify-between items-center pb-8 mt-4 no-print">
          {!shareMode && (
            <button
              onClick={() => navigate('/dashboard')}
          ...
          <div className="flex gap-2 ml-auto">
            <button
              onClick={() => window.print()}
              className="flex items-center gap-1.5 px-4 py-2 border border-slate-200 rounded-xl text-sm hover:border-[#0f2044] transition-colors"
            >
              <Printer className="w-4 h-4" /> Print
            </button>
            <button
              onClick={handleSavePdf}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#0f2044] text-white rounded-xl text-sm hover:bg-[#162d4a] transition-colors"
            >
              <Download className="w-4 h-4" /> Save PDF
            </button>
            {!shareMode && params.id && (
              <button
                onClick={() => navigate(`/records/${params.id}/edit`)}
                className="flex items-center gap-1.5 px-4 py-2 border border-[#0f2044] text-[#0f2044] rounded-xl text-sm hover:bg-blue-50 transition-colors"
              >
