...
import { ArrowLeft, Save, CheckCircle2, Loader2, Zap, FileDown, Share2, ChevronDown, ChevronUp } from 'lucide-react';
...
const EMPTY_FORM = {
  status: 'Draft', date: new Date().toLocaleDateString('en-AU'),
  addressLocation: '', switchboardNumber: '', contractorName: '', contractorNumber: '',
  workerName: '', licenceNumber: '', clientEmail: '', pscAtMainSwitch: '',
  livePartsScreened: '', mainLinkNeutralReconnected: '', signatureData: '',
  ...
  msbMenCompliant: '', msbMaxDemand: '', msbMainSwitchCurrentRating: '', msbMainSwitchPscRating: '',
  msbConductorCcc: '', msbConductorSize: '', msbEarthContMain: '', msbEarthContEq: '',
  msbPolarity: '', msbInsResAE: '', msbInsResAN: '', msbInsResNE: '', msbInsResPP: '',
  msbCircuitLength: '', msbComments: '', companyName: '', companyAbn: '', companyPhone: '', companyAddress: '', notes: '',
};
...
function Field({ label, children, className = '' }) {
  return <div className={`space-y-1 ${className}`}><label className="text-xs font-semibold text-slate-600 uppercase tracking-wide block">{label}</label>{children}</div>;
}
function TF({ id, value, onChange, placeholder = '', type = 'text' }) {
  return <input type={type} value={value || ''} onChange={(e) => onChange(id, e.target.value)} placeholder={placeholder}
...
export default function RecordForm() {
  ...
  useEffect(() => {
    if (isNew) {
      try {
        ...
        if (draft) { const parsed = JSON.parse(draft); setForm({ ...EMPTY_FORM, ...parsed.form }); if (parsed.mainsRows) setMainsRows(parsed.mainsRows); if (parsed.subRows) setSubRows(parsed.subRows); }
      } catch {}
      return;
    }
    getRecord(id).then((data) => {
      ...
      if (data.companyName) setShowBranding(true);
      setLoading(false);
    });
  }, [id, isNew]);
  ...
  useEffect(() => {
    if (!isNew) return;
    const interval = setInterval(() => { localStorage.setItem(DRAFT_KEY, JSON.stringify({ form, mainsRows, subRows })); }, 30000);
  ...
  const handleCopyShareLink = async () => {
    ...
    setCopied(true); showToast('Shareable link copied!'); setTimeout(() => setCopied(false), 2500);
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-5 overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wide"><th className="text-left px-3 py-2">#</th><th className="text-left px-3 py-2">Type / Description</th><th className="text-left px-3 py-2">Serial Number</th><th className="text-left px-3 py-2">Cal. Date</th></tr></thead>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <Field label="Comments" className="col-span-2 lg:col-span-4">
              <textarea value={form.msbComments || ''} onChange={(e) => setField('msbComments', e.target.value)} rows={2} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-[#0f2044] focus:ring-1 focus:ring-[#0f2044]/20" />
            </Field>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <p className="text-xs text-slate-600 bg-slate-50 rounded-lg p-3 border border-slate-200">I declare that the electrical installation described above has been tested in accordance with AS/NZS 3000:2018 and AS/NZS 3017 and the results are as recorded above.</p>
            ...
            <Field label="Notes / Defects">
              <textarea value={form.notes || ''} onChange={(e) => setField('notes', e.target.value)} rows={3} placeholder="Any defects, observations or notes..."
        ...
        <div className="flex justify-between items-center pb-8">
          <button onClick={() => navigate('/dashboard')} className="text-slate-400 hover:text-slate-600 text-sm flex items-center gap-1"><ArrowLeft className="w-4 h-4" /> Back to dashboard</button>
