...
Deno.serve(async (req) => {
  try {
    ...
    // Use Resend onboarding sender until testsheet.com.au domain is verified in Resend dashboard
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        <tr><td style="background:#f1f5f9;padding:20px 32px;border-top:1px solid #e2e8f0;">
          <p style="margin:0;color:#94a3b8;font-size:12px;">Sent by ${fromName} via TestSheet — testsheet.com.au</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `${fromName} <${senderAddress}>`, to: [to], subject: `Electrical Test Results — ${addressLocation || 'Job'}`, html: emailHtml }),
    });
